...
class LexicalAnalyzer:
    ...
    def process_current_line_tokens(self):
        if self.current_line and self.current_col == len(self.current_line):
            self.read_next_line()
            self.current_line_number += 1
            self.current_col = 0
        line = self.current_line

        new_col, string, col, line_size, tabs = 1, "", self.current_col, len(line), 0
        ...
        while col < line_size:
            ...
            if c == '\'' or c == '"':  # string and character literals
                ...
                return Token(TokenPosition(self.current_line_number, new_col), category, string)
                string, new_col = "", col + 1 + (self.TAB_SIZE * tabs)
            elif self.is_separator(c):
                if string:
                    self.token_buffer.append(
                        Token(TokenPosition(self.current_line_number, new_col), self.get_category(string), string))
                if c != ' ' and c != '\t':
                    ...
                    if c == '/':
                        ...
                        if col < line_size and line[col] == '/':
                            return True
                        col -= 1
                    self.token_buffer.append(Token(TokenPosition(self.current_line_number, new_col), self.separators[c], c))
